...
void RRGLWidget::drawGrid() {
  ...
  glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE, color);//设置线条的颜色
  int step = 50;
  int num = 15;
  for (int i = -num; i < num + 1; i++) {
    ...
    glVertex3f(num * step, i * step, 0);
    glEnd();//glBegin 和 glEnd 配合 glVertex3f 来绘制每条线。
  }
  glPopMatrix();
}
...
void RRGLWidget::setupColor(int r, int g, int b) {
   ...
   */
  GLfloat color[] = {r / 255.0, g / 255.0, b / 255.0};//设置绘制颜色
  glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE, color);//设置材质的环境和漫反射颜色
}
//控制旋转
